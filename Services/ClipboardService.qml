...
Singleton {
  ...
  // Public API
  property bool active: Settings.data.launcher.enableClipboardHistory && cliphistAvailable
  ...
  Process {
    ...
    onExited: (exitCode, exitStatus) => {
      ...
      if (exitCode === 0) {
        ...
        Logger.log("ClipboardService", "cliphist is available")
        ...
        if (Settings.data.launcher.enableClipboardHistory) {
          Logger.warn("ClipboardService", "cliphist not found - clipboard history disabled")
          // Note: ToastService might not be initialized yet, so skip toast
          // User will see disabled state in launcher instead
        }
      }
    }
  }
    ...
    onExited: (exitCode, exitStatus) => {
      ...
      const parsed = lines.map(l => {
                                 ...
                                 if (!root.firstSeenById[id]) {
                                   root.firstSeenById[id] = Date.now() / 1000
                                 }
                                 return {
                                   "id": id,
                                   "preview": preview,
  ...
  function startWatchers() {
    ...
    Logger.log("ClipboardService", "Starting clipboard watchers")
    // Start text watcher
  ...
  function stopWatchers() {
    if (!watchersStarted)
      return
    Logger.log("ClipboardService", "Stopping clipboard watchers")
